"""Local survival planning with bounded sonar cooperation."""
...
class Planner:
    ...
    def try_reproduce(self, ct, game, origin, blocked):
        ...
        ct.set_indicator_string(f'v4 breed open={self.open_score:.2f} '
    ...
    def choose(self, ct, game):
        ...
        pressured, retire = self.population_pressure(ct, origin, blocked)
        self.sonar.prepare(self, ct, turn, origin, blocked)
        retire = retire and self.sonar.may_retire(self, ct, turn)
        if (not pressured and self.sonar.request is None
        ...
        for index, dest in enumerate(self.graph[origin]):
            ...
            if best is None or score > best[0]:
                ...
                best_info = (f'v4 room={room} safe={safe_room} exits={len(exits)} '
                             f'risk={direct}/{sprint} food={food_distance} '
                             f'intel={len(self.sonar.reports)}')
        if best is not None:
            ...
            ct.set_indicator_string('v4 no verified move: emergency split')
            ct.do_split(2)
            self.sonar.emit(self, ct, turn, origin, origin, DIRS)
        else:
            ct.set_indicator_string('v4 yield: crowded short snake, no rescue'
                                    if retire else
                                    'v4 no verified move: uncertain fallback')
