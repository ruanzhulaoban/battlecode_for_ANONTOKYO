"""Local exploration and pressure-aware population control."""
...
class Planner:
    ...
    def try_reproduce(self, ct, game, origin, blocked):
        ...
        ct.set_indicator_string(f'v3 breed open={self.open_score:.2f} '
    ...
    def choose(self, ct, game):
        ...
        if not pressured and self.try_reproduce(ct, game, origin, blocked):
            return
        heading = ct.get_dir()
        ...
        for index, dest in enumerate(self.graph[origin]):
            ...
            if best is None or score > best[0]:
                ...
                best_info = (f'v3 room={room} safe={safe_room} exits={len(exits)} '
        ...
        if best is not None:
            ...
            ct.set_indicator_string('v3 no verified move: emergency split')
            ct.do_split(2)
        else:
            ct.set_indicator_string('v3 yield: crowded short snake, no rescue'
                                    if retire else
                                    'v3 no verified move: uncertain fallback')
